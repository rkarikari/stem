...
_AB='total_songs'
_AA='auto_mode_stats'
_A9='auto_mode_active'
_A8='current_song_result'
_A7='https://github.com/rkarikari/RadioSport-chat'
_A6='RadioSport AI'
_A5='http://localhost:11434'
_A4='https://openrouter.ai/api/v1/models'
_A3='Cloud'
_A2='original_prompt'
_A1='reason'
_A0='Deep Dive'
_z='Detailed Analysis'
_y='🔍 Identifying song...'
_x='subtitle'
_w='✅ Recording completed!'
_v='sample_rate'
_u='Released'
_t='metadata'
_s='SONG'
...
_b='name'
_a='Local'
_Z='document_cache_misses'
_Y='model_cache_misses'
_X='start_time'
_W='code'
_V='ollama_host'
_U='document_cache_hits'
_T='ignore'
_S='user'
_R='model_cache_hits'
_Q='type'
...
warnings.filterwarnings(_T,category=DeprecationWarning)
os.environ['PYTHONWARNINGS']=_T
...
OPENROUTER_MODELS_URL=_A4
DEFAULT_OLLAMA_HOST=_A5
...
CODE_EXTENSIONS={'.py':'python','.js':'javascript','.ts':'typescript','.cpp':'cpp','.c':'c','.java':'java','.html':'html','.css':'css','.json':'json','.md':'markdown','.sh':_n,'.sql':'sql','.bat':'batch'}
IMAGE_EXTENSIONS='.png','.jpg','.jpeg'
st.set_page_config(page_title=_A6,page_icon='🧟🎵',layout='wide',menu_items={'Report a Bug':_A7,'About':'Copyright © RNK, 2025 RadioSport. All rights reserved.'})
def initialize_session_state():
	...
	if _AF not in st.session_state:st.session_state.api_provider=_a
...
def update_all_ollama_host_references(new_host):
	...
	for(module_name,module)in sys.modules.items():
		...
		if hasattr(module,_V):old_value=getattr(module,_V);setattr(module,_V,new_host);modules_updated.append(f"{module_name}.ollama_host: {old_value} -> {new_host}")
	for(module_name,module)in sys.modules.items():
...
def get_ollama_models():
	...
	if end_time-start_time<.1 and result:st.session_state.cache_stats[_R]+=1
	else:st.session_state.cache_stats[_Y]+=1
	return result
...
def split_message_cached(content):
	...
	for match in CODE_BLOCK_PATTERN.finditer(content):
		...
		if last_end<start:
			...
			if text_content.strip():parts.append({_Q:_L,_E:text_content})
		language=match.group(1)or _L;code=match.group(2);parts.append({_Q:_W,'language':language,_W:code});last_end=end
	if last_end<len(content):
		remaining_content=content[last_end:]
		if remaining_content.strip():parts.append({_Q:_L,_E:remaining_content})
	return parts
...
def process_document(file_content,file_type,file_name):
	...
	st.session_state.doc_cache[cache_key]=result;st.session_state.cache_stats[_Z]+=1;return result
def get_base64_image(file_content):
	...
	base64_str=base64.b64encode(file_content).decode(_c);st.session_state.base64_cache[content_hash]=base64_str
...
def get_load_balanced_api_key(provider):
	...
	if provider not in st.session_state.api_usage:st.session_state.api_usage[provider]={A:0,_h:set(),B:{}}
	usage=st.session_state.api_usage[provider];valid_keys=[k for k in keys if k not in usage[_h]]
	if not valid_keys:usage[_h].clear();valid_keys=keys
...
def get_free_openrouter_models():
	...
	try:
		response=requests.get(_A4,headers={'User-Agent':'RadioSport-AI/1.0','Accept':_q},timeout=30);response.raise_for_status();models_data=response.json();free_models=[]
...
def get_song_history_context():
	...
	for(idx,song)in enumerate(st.session_state.identified_songs,1):
		...
		if _G in song and _H in song[_G]:
			track=song[_G][_H]
			if _J in track:genre=track[_J].get(_d,_D);context+=f"   - Genre: {genre}\n"
			if _P in track:
				for section in track[_P]:
					if section.get(_Q)==_s:
						metadata=section.get(_t,[])
						for meta in metadata:
							if meta.get(_F)=='Album':context+=f"   - Album: {meta.get(_L,_D)}\n"
							elif meta.get(_F)==_u:release_year=meta.get(_L,_D);context+=f"   - Released: {release_year}\n"
		context+=_g
...
def prepare_song_history_data():
	...
	data={C:[],_J:{},A:{},B:{},_AB:len(st.session_state.identified_songs)}
	for song in st.session_state.identified_songs:
		song_data={_F:song[_F],_I:song[_I],_M:song[_M]};genre=_D
		if _G in song and _H in song[_G]:
			track=song[_G][_H]
			if _J in track:genre=track[_J].get(_d,_D)
	...
	try:
		...
		for i in range(device_count):
			try:
				...
				if device_info.get(C)>0:devices.append({A:i,_b:device_info.get(_b,f"Device {i}"),B:device_info.get(C),_v:device_info.get('defaultSampleRate'),'host_api':device_info.get('hostApi')})
			except:continue
...
def record_audio_browser_advanced(duration=RECORD_SECONDS,show_progress=_B):
	...
	if audio_bytes:
		temp_file=tempfile.NamedTemporaryFile(delete=_C,suffix=_k);temp_file.write(audio_bytes);temp_file.close()
		if show_progress:st.success(_w)
		return temp_file.name
...
async def identify_song(audio_file_path):
	...
	try:
		with warnings.catch_warnings():
			warnings.simplefilter(_T);shazam=Shazam()
			...
			for section in track[_P]:
				if section.get(_Q)==_s:
					...
					for meta in metadata:
						...
						elif meta.get(_F)==_u:released=meta.get(_L,_D)
		st.markdown(f"**💿 Album:** {album}");st.markdown(f"**📅 Released:** {released}")
		if _J in track:genres=track[_J].get(_d,_D);st.markdown(f"**🎭 Genre:** {genres}")
					...
					for(i,song)in enumerate(songs[1:],1):
						...
						with st.container():
							...
							with song_col1:
								...
								if len(title)>30:title=title[:27]+_m
								if len(artist)>25:artist=artist[:22]+_m
			...
			if time_to_next<=0 and st.session_state.auto_mode_active:
				...
				try:
					...
					if audio_file and os.path.exists(audio_file):
						status_placeholder.info(_y);loop=asyncio.new_event_loop();asyncio.set_event_loop(loop)
						try:result=loop.run_until_complete(identify_song(audio_file))
						finally:loop.close()
		...
		if st.session_state.identified_songs:
			...
			for(i,song)in enumerate(recent_songs):
				with st.container():
					...
					if len(title)>25:title=title[:22]+_m
					if len(artist)>20:artist=artist[:17]+_m
...
def main_tab():
	...
	with col2:
		...
		if hasattr(st.session_state,_A8)and st.session_state.current_song_result:display_song_info(st.session_state.current_song_result)
...
def generate_analysis_prompt(depth):
	...
	if depth==_f:return'Provide a quick overview of my song history. Highlight key trends and top artists.'
	elif depth==_z:return'Provide a detailed analysis of my song history. Include trends in genres, time periods, and artist diversity. Also analyze listener personality and musical preferences.'
	elif depth==_A0:return'Perform a deep dive analysis of my song history. Explore emotional resonance, social context, lifestyle alignment, and musical sophistication. Include recommendations for similar artists.'
...
def generate_ai_analysis_response(response_text):
	...
	if rec_match:
		...
		for item in rec_items:
			...
			if title_match and artist_match:rec={_F:title_match.group(1),_I:artist_match.group(1).strip(),_A1:reason_match.group(1).strip()if reason_match else'No reason provided'};recommendations.append(rec)
	...
	with st.sidebar:
		...
		if api_provider==_a:
			...
			if not available_models:
				...
				st.stop()
			model_display=[f"{model.get(_b,model[_j])}"for model in available_models];col1,col2=st.columns([6,1])
			with col1:
		...
		with st.expander('📊 Cache Statistics'):
            ...
            <div class="cache-stats">
                <table>
                    ...
                    <tr><td>Hit Rate</td><td class="{C if model_hit_rate>50 else D}">{model_hit_rate:.1f}%</td></tr>
                    <tr><td>Hits</td><td class="cache-hit">{stats[_R]}</td></tr>
                    <tr><td>Misses</td><td class="cache-miss">{stats[_Y]}</td></tr>
                    <tr><td colspan="2"><strong>Document Cache</strong></td></tr>
                    <tr><td>Hit Rate</td><td class="{C if doc_hit_rate>50 else D}">{doc_hit_rate:.1f}%</td></tr>
                    <tr><td>Hits</td><td class="cache-hit">{stats[_U]}</td></tr>
                    <tr><td>Misses</td><td class="cache-miss">{stats[_Z]}</td></tr>
