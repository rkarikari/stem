...
if'drone_location'not in st.session_state:st.session_state.drone_location=_U
...
def calculate_two_ray_model(distance_km,freq_mhz,h_tx_m,h_rx_m):
	'\n    Two-Ray Ground Reflection Model - FULLY FREQUENCY-DEPENDENT\n    \n    Key physics:\n    1. Critical distance depends on wavelength (frequency-dependent)\n    2. Ground reflection coefficient decreases with frequency\n    3. At all distances, higher frequency = more path loss\n    \n    Returns path loss in dB (always frequency-dependent)\n    ';D=h_rx_m;C=h_tx_m;B=freq_mhz;A=distance_km
	if A<=.001:return 0
	E=A*1000;J=3e8/(B*1e6)
	if C<=0 or D<=0:return calculate_fspl_db(A,B)
	G=4*np.pi*C*D/J;K=40*np.log10(E)-10*np.log10(C**2*D**2);H=B/15e1;L=2.*np.log10(H);M=np.log10(max(A,_C));N=.5*M*np.log10(H);F=K+L+N
...
def calculate_okumura_hata(distance_km,freq_mhz,h_tx_m,h_rx_m=2.,environment=_D):
	'\n    Okumura-Hata Model - STANDARD ITU-R P.529 IMPLEMENTATION\n    \n    Valid ranges:\n    - Frequency: 150-1500 MHz\n    - Distance: 1-20 km\n    - Base height: 30-200 m\n    - Mobile height: 1-10 m\n    ';G=environment;F=h_rx_m;C=h_tx_m;B=distance_km;A=freq_mhz
	if B<=.001:return 0
	if B<_C:return calculate_fspl_db(B,A)
	if A<150 or A>1500:return calculate_two_ray_model(B,A,C,F)
	if C<30 or C>200:return calculate_two_ray_model(B,A,C,F)
	K=np.clip(B,_C,2e1);H=C;I=np.clip(F,1,10)
	if A<200:J=(1.1*np.log10(A)-.7)*I-(1.56*np.log10(A)-.8)
	else:J=3.2*np.log10(11.75*I)**2-4.97
	D=69.55+26.16*np.log10(A)-13.82*np.log10(H)-J+(44.9-6.55*np.log10(H))*np.log10(K)
	if G==_J:E=D
	elif G==_D:E=D-2*np.log10(A/28)**2-5.4
	elif G==_K:E=D-4.78*np.log10(A)**2+18.33*np.log10(A)-40.94
	else:E=D-4.78*np.log10(A)**2+18.33*np.log10(A)-40.94
	return max(E,0)
@st.cache_data(ttl=1)
def calculate_itu_p1546(distance_km,freq_mhz,h_tx_m,time_percent=50,environment=_D):
	'\n    ITU-R P.1546 SIMPLIFIED APPROXIMATION\n    \n    Valid: 30-4000 MHz, 1-1000 km, heights < 3000 m\n    ';H=environment;G=h_tx_m;C=distance_km;B=time_percent;A=freq_mhz
	if C<=.001:return 0
	if C<_C:return calculate_okumura_hata(C,A,G,10,H)
	K=np.clip(C,_C,1e3);F=np.clip(G,10,3000);I=10
	if A<200:J=(1.1*np.log10(A)-.7)*I-(1.56*np.log10(A)-.8)
	else:J=3.2*np.log10(11.75*I)**2-4.97
	L=69.55+26.16*np.log10(A)-13.82*np.log10(F)-J+(44.9-6.55*np.log10(F))*np.log10(K)
	if A<150:D={_J:12,_D:8,_K:4,_N:2}
	elif A<300:D={_J:15,_D:10,_K:5,_N:3}
	elif A<600:D={_J:18,_D:12,_K:6,_N:3}
	else:D={_J:20,_D:13,_K:7,_N:4}
	M=D.get(H,10);N=min(F/5e1,_C);O=M*(1-.5*N)
	if B>=50:E=0
	elif B>=10:E=-(50-B)/4e1*7.
	elif B>=1:E=-7.-(10-B)/9.*8.
	else:E=-15.
	P=L+O+E;return max(P,0)
@st.cache_data(ttl=1)
def calculate_path_loss_db(distance_km,freq_mhz,n=2.,altitude_m=0,propagation_model=_E,h_rx_m=2.,environment=_D,time_percent=50,_cache_version=CACHE_VERSION):
	...
	if A<=.001:return 0
	if C==_V:return calculate_fspl_db(A,B,G)
	elif C==_W:return calculate_two_ray_model(A,B,D,E)
...
@st.cache_data
def calculate_received_power(tx_power_w,tx_gain_dbi,rx_gain_dbi,distance_km,freq_mhz,n=2.,additional_loss_db=0,swr=_C,altitude_m=0,propagation_model=_L,h_rx_m=2.,environment=_D,time_percent=50,polarization_mismatch=0,antenna_efficiency=.9):
...
@st.cache_data
def calculate_range(tx_power_w,tx_gain_dbi,rx_gain_dbi,rx_sensitivity_dbm,freq_mhz,n=2.,additional_loss_db=0,swr=_C,fade_margin_db=0,altitude_m=0,propagation_model=_L,h_rx_m=2.,environment=_D,time_percent=50,polarization_mismatch=0,antenna_efficiency=.9):
...
@st.cache_data
def validate_propagation_physics(freq_vhf,freq_uhf,altitude_m,propagation_model,environment,ground_rx_height,time_percent):'\n    Validate that propagation physics are correct:\n    - VHF should have LESS path loss than UHF at same distance\n    - Higher frequency = more loss (always)\n    \n    Returns: (is_valid, vhf_loss, uhf_loss, difference, expected_diff)\n    ';G=time_percent;F=ground_rx_height;E=environment;D=propagation_model;C=altitude_m;B=freq_uhf;A=freq_vhf;H=1e1;I=calculate_path_loss_db(H,A,2.,C,D,F,E,G);J=calculate_path_loss_db(H,B,2.,C,D,F,E,G);L=20*np.log10(B/A);K=J-I;M=K>0;return M,I,J,K,L
...
with st.sidebar.expander('🌍 Environment & Model',expanded=_A):
	propagation_model=st.selectbox(_e,[_H,_E,_W,_V,_X],index=0,format_func=lambda x:{_H:'ITU-R P.1546 (Recommended)',_E:'Okumura-Hata (1-20km)',_W:'Two-Ray Ground (LOS)',_V:'Free Space (Ideal)',_X:'Simple (Path Loss Exp.)'}[x],key='prop_model')
	if propagation_model in[_X,_H,_L]:path_loss_exponent=st.slider('PL Exponent',2.,4.5,2.,.1,key='pl_exp')
	else:path_loss_exponent=2.
	if propagation_model in[_E,_H,_L]:environment=st.selectbox(_A0,[_D,_J,_K,_N],key='env')
	else:environment=_D
...
is_valid,pl_vhf,pl_uhf,diff,expected=validate_propagation_physics(freq_2m,freq_70cm,drone_altitude,propagation_model,environment,ground_rx_height,time_percent)
if not is_valid:st.error(f"⚠️ PHYSICS ERROR: UHF has {abs(diff):.1f} dB LESS loss than VHF (impossible!)");st.error(f"Expected: UHF should have ~{expected:.1f} dB MORE loss than VHF");st.error('Please report this as a bug with your current settings.')
...
with tab1:
	...
	with col_map:
		...
		if st.session_state.drone_location is _U:map_center=[5.6037,-.187]
		...
		if st.session_state.drone_location is not _U:folium.Marker(st.session_state.drone_location,popup=f"""
                <b>{APP_NAME}</b><br>
                ...
                """,tooltip='Drone Repeater Station',icon=folium.Icon(color=_P,icon='broadcast-tower',prefix='fa')).add_to(m);folium.Circle(st.session_state.drone_location,radius=range_2m*1000,popup=f"2m Band: {range_2m:.1f} km",color=_F,fill=_A,fillColor=_F,fillOpacity=.15,weight=2,dashArray='5, 5').add_to(m);folium.Circle(st.session_state.drone_location,radius=range_70cm*1000,popup=f"70cm Band: {range_70cm:.1f} km",color=_M,fill=_A,fillColor=_M,fillOpacity=.1,weight=2,dashArray='10, 5').add_to(m);folium.Circle(st.session_state.drone_location,radius=system_range*1000,popup=f"System Range: {system_range:.1f} km",color=_Q,fill=_A,fillColor=_Q,fillOpacity=.2,weight=3).add_to(m);folium.Circle(st.session_state.drone_location,radius=radio_horizon*1000,popup=f"Radio Horizon: {radio_horizon:.1f} km",color=_Y,fill=_G,weight=1,dashArray='2, 5').add_to(m)
		...
		with col_action:
			if st.button('🗑️ Clear Location'):st.session_state.drone_location=_U;st.rerun()
	with col_info:
...
with tab2:
	...
	st.markdown('**📉 Received Power vs Distance**');distances=np.linspace(.1,min(max(range_2m,range_70cm)*1.5,100),200);rx_powers_2m=[calculate_received_power(tx_power_2m,antenna_gain,antenna_gain,A,freq_2m,path_loss_exponent,total_additional_loss,swr_2m,drone_altitude,propagation_model,ground_rx_height,environment,time_percent,polarization_mismatch,antenna_efficiency)for A in distances];rx_powers_70cm=[calculate_received_power(tx_power_70cm,antenna_gain,antenna_gain,A,freq_70cm,path_loss_exponent,total_additional_loss,swr_70cm,drone_altitude,propagation_model,ground_rx_height,environment,time_percent,polarization_mismatch,antenna_efficiency)for A in distances];fig=go.Figure();fig.add_trace(go.Scatter(x=distances,y=rx_powers_2m,mode='lines',name=f"2m ({freq_2m} MHz)",line=dict(color=_F,width=2)));fig.add_trace(go.Scatter(x=distances,y=rx_powers_70cm,mode='lines',name=f"70cm ({freq_70cm} MHz)",line=dict(color=_M,width=2)));fig.add_hline(y=effective_sensitivity,line_dash=_R,line_color=_P,annotation_text='Effective Sensitivity',annotation_position='right');fig.add_hline(y=effective_sensitivity+total_fade_margin,line_dash='dot',line_color=_Y,annotation_text=f"Target ({link_availability:.1f}% Avail)",annotation_position='right');fig.add_vline(x=range_2m,line_dash=_R,line_color=_F,annotation_text=f"2m: {range_2m:.1f}km");fig.add_vline(x=range_70cm,line_dash=_R,line_color=_M,annotation_text=f"70cm: {range_70cm:.1f}km");fig.update_layout(xaxis_title=_l,yaxis_title='RX Power (dBm)',hovermode=_b,height=350,margin=dict(t=20,b=40,l=40,r=20),title=f"Propagation Model: {propagation_model.upper()} | Environment: {environment}");st.plotly_chart(fig,width=_B)
with tab3:
	col1,col2=st.columns(2)
	with col1:
		st.markdown('**Power vs Range**');fig_power=go.Figure();fig_power.add_trace(go.Scatter(x=power_levels,y=ranges_2m,mode=_S,name='2m',line=dict(color=_F,width=2),marker=dict(size=4)));fig_power.add_trace(go.Scatter(x=[tx_power_2m],y=[range_2m],mode=_m,name='Current',marker=dict(size=12,color=_P,symbol=_n)))
		if len(ranges_2m)>0:
		...
		fig_power.update_layout(xaxis_title='TX Power (W)',yaxis_title=_o,hovermode=_b,height=350,margin=dict(t=20,b=40,l=40,r=20),title=f"Availability: {link_availability:.1f}%");st.plotly_chart(fig_power,width=_B);power_efficiency=range_2m/tx_power_2m if tx_power_2m>0 else 0;st.metric(_AA,f"{power_efficiency:.1f} km/W")
	with col2:
	...
	fig_models=go.Figure();fig_models.add_trace(go.Bar(x=model_names,y=model_ranges,marker_color=[_F,_M,_Y,_P,_Q]));current_model_idx=models.index(propagation_model)if propagation_model in models else 0;fig_models.add_hline(y=range_2m,line_dash=_R,line_color=_Q,annotation_text=f"Current: {range_2m:.1f} km");fig_models.update_layout(xaxis_title=_e,yaxis_title=_o,height=300,margin=dict(t=20,b=40,l=40,r=20),title='Comparison of Different Propagation Models');st.plotly_chart(fig_models,width=_B)
with tab4:
	...
	if propagation_model==_H or propagation_model==_L:recommendations.append(f"• **Propagation Model**: Using {propagation_model.upper()} with v2.3.5 FINALLY FIXED physics");scores.append(3)
...
with tab5:
	col1,col2=st.columns(2)
	with col1:st.markdown('**Configuration Summary**');config_data={'Category':['Radio','Radio','Antenna','Antenna',_r,_r,_r,'Drone','Drone',_A0,'Propagation'],_Z:['2m TX Power','70cm TX Power','Gain',_v,_i,_AC,'Desense','Altitude','RX Height','Atmos Loss','Model'],_q:[f"{tx_power_2m:.2f}W",f"{tx_power_70cm:.2f}W",f"{antenna_gain:+.0f}dBi",antenna_polarization,f"{noise_figure:.0f}dB",f"{if_bandwidth_khz:.1f}kHz",f"{desense_penalty:.0f}dB",f"{drone_altitude}m",f"{ground_rx_height}m",f"{atmospheric_loss:.2f}dB/km",propagation_model.upper()]};df_config=pd.DataFrame(config_data);st.dataframe(df_config,hide_index=_A,width=_B,height=400)
	with col2:
	...
	st.markdown(_O);col3,col4=st.columns(2)
...
with tab6:
	...
	with col1:st.dataframe(debug_df,hide_index=_A,width=_B);import plotly.graph_objects as go;fig=go.Figure();fig.add_trace(go.Scatter(x=test_distances,y=pl_2m_tests,mode=_S,name='2m (VHF)',line=dict(color=_F,width=2)));fig.add_trace(go.Scatter(x=test_distances,y=pl_70cm_tests,mode=_S,name='70cm (UHF)',line=dict(color=_M,width=2)));fig.update_layout(title='Path Loss vs Distance (Both Bands)',xaxis_title=_l,yaxis_title='Path Loss (dB)',hovermode=_b,height=400);st.plotly_chart(fig,width=_B)
	with col2:
		...
		if .5*expected_fspl_diff<=avg_diff<=1.5*expected_fspl_diff:st.success(f"✅ Within expected range ({.5*expected_fspl_diff:.1f} - {1.5*expected_fspl_diff:.1f} dB)")
		else:st.warning('⚠️ Difference is outside typical range')
		if range_70cm>0:
		...
		st.markdown(_O);st.markdown('**Current Model:**');st.text(f"{propagation_model.upper()}");st.text(f"Environment: {environment}");st.text(f"Altitude: {drone_altitude}m")
with tab7:
	...
	st.markdown(_O);col_guide,col_tech=st.columns(2)
	with col_guide:
...
- **VHF/UHF Range Ratio**: {range_2m/range_70cm if range_70cm>0 else _c:.2f}:1
- **Expected Ratio at {drone_altitude}m**: {1.1 if drone_altitude>=75 else 1.2:.1f}-{1.3 if drone_altitude>=75 else 1.4:.1f}:1
- **Status**: {"✅ PHYSICS FINALLY FIXED v2.3.5 - Within expected range"if(1.1 if drone_altitude>=75 else 1.2)<=range_2m/range_70cm<=(1.3 if drone_altitude>=75 else 1.4)else"⚠️ Physics check required"if range_70cm>0 else _c}
  ...
  <Document>
    ...
    <Placemark>
      ...
      <description>
        ...
        VHF/UHF Ratio: {range_2m/range_70cm if range_70cm>0 else _c:.2f}
      </description>
      <Point>
...
Horizon Utilization: {range_2m/radio_horizon*100 if radio_horizon>0 else 0:.0f}%
VHF/UHF Ratio: {range_2m/range_70cm if range_70cm>0 else _c:.2f}:1 (v2.3.5 FINALLY FIXED physics)
