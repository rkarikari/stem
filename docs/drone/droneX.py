...
if(pd.Timestamp.now()-st.session_state.last_cleanup).total_seconds()>300:
	...
	st.session_state.last_cleanup=pd.Timestamp.now()_AA='70cm Range'
_A9='System Range'
_A8='Bandwidth'
_A7='Noise Figure'
_A6='Altitude Efficiency'
...
@st.cache_data
def calculate_itu_p1546(distance_km,freq_mhz,h_tx_m,time_percent=50):
	'\n    Calculate path loss using ITU-R P.1546 model\n    Based on empirical field strength curves at 100, 600, 2000 MHz\n    ';E=time_percent;D=freq_mhz;C=distance_km
	if C<=.001:return 0
	H=min(max(C,1),1000);A=max(h_tx_m,10);F=106.9-20*np.log10(H)
	if A>10:
		if A<=50:G=10*np.log10(A/10)
		else:G=15*np.log10(A/10)
		F+=G
	if E>=50:B=0
	elif E>=10:B=8
	elif E>=1:B=15
	else:B=25
	I=20*np.log10(600/max(D,30));J=F+I+B;K=139.3-J+20*np.log10(D)-77.2;return max(K,calculate_fspl_db(C,D))
@st.cache_data
def calculate_vhf_advantage(distance_km,freq_mhz):'\n    [DEPRECATED in v1.2.0]\n    VHF advantage now handled by propagation models directly.\n    This function kept for API compatibility but returns 0.\n    ';return 0
@st.cache_data
def calculate_uhf_penalty(distance_km,freq_mhz,environment=_D):'\n    [DEPRECATED in v1.2.0]\n    UHF penalty now handled by propagation models directly.\n    This function kept for API compatibility but returns 0.\n    ';return 0
