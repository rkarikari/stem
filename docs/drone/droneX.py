...
def calculate_itu_p1546(distance_km,freq_mhz,h_tx_m,time_percent=50,environment=_D):
	...
	if G<=.001:return 0
	K=min(max(G,1),1000);I=max(h_tx_m,10);L=69.55+26.16*np.log10(A)-13.82*np.log10(I)+(44.9-6.55*np.log10(I))*np.log10(K);J=min(I/1e2,_B);E=np.exp(-2.5*J)
	if A<=100:B=-7.;C=B*E
	elif A<=200:D=(A-100)/100;B=-7.+3.*D;C=B*E
	elif A<=400:D=(A-200)/200;B=-4.+2.*D;C=B*E
	elif A<=600:D=(A-400)/200;B=-1.5+.5*D;C=B*E
	else:C=0
	M={_Y:1.1,_D:_B,'rural':.9,'open':.8}.get(environment,_B);N=_B-.5*J;C*=M*N
	if H>=50:F=0
	elif H>=10:F=-3
	elif H>=1:F=-6
	else:F=-10
...
def calculate_path_loss_db(distance_km,freq_mhz,n=2.,altitude_m=0,propagation_model=_E,h_rx_m=2.,environment=_D,time_percent=50):
	...
	elif E==_S:C=calculate_two_ray_model(A,B,D,F)
	...
	else:K=calculate_fspl_db(_B,B);C=K+10*n*np.log10(A)
...
@st.cache_data
def calculate_received_power(tx_power_w,tx_gain_dbi,rx_gain_dbi,distance_km,freq_mhz,n=2.,additional_loss_db=0,swr=_B,altitude_m=0,propagation_model=_E,h_rx_m=2.,environment=_D,time_percent=50,polarization_mismatch=0,antenna_efficiency=.9):
	'Calculate received power with all losses considered';B=antenna_efficiency;A=distance_km
	if A<=.001:return 100
	C=10*np.log10(swr)if swr>_B else 0;D=10*np.log10(tx_power_w*1000);E=-10*np.log10(B)if B<_B else 0;F=calculate_path_loss_db(A,freq_mhz,n,altitude_m,propagation_model,h_rx_m,environment,time_percent);G=F+additional_loss_db+C+polarization_mismatch+E;H=D+tx_gain_dbi+rx_gain_dbi-G;return H
@st.cache_data
...
@st.cache_data
def calculate_range(tx_power_w,tx_gain_dbi,rx_gain_dbi,rx_sensitivity_dbm,freq_mhz,n=2.,additional_loss_db=0,swr=_B,fade_margin_db=0,altitude_m=0,propagation_model=_E,h_rx_m=2.,environment=_D,time_percent=50,polarization_mismatch=0,antenna_efficiency=.9):
	...
	if E>=_B:
		W=calculate_received_power(B,F,G,_B,H,n,I,J,C,K,L,M,N,O,P)
		if W<Q:E=_B
	while E-D>U and S<V:
...
with st.sidebar.expander('📻 Radio Parameters',expanded=_A):
	...
	with col1:tx_power_2m=st.slider('2m TX (W)',.1,5.,2.2,.05,key='tx2m',help='Max 5W per specs');swr_2m=st.slider('2m SWR',_B,3.,1.3,.1,key='swr2m')
	with col2:tx_power_70cm=st.slider('70cm TX (W)',.1,4.,2.2,.05,key='tx70cm',help='Max 4W per specs');swr_70cm=st.slider('70cm SWR',_B,3.,1.3,.1,key='swr70cm')
with st.sidebar.expander('📶 Antenna System',expanded=_A):
	antenna_gain=st.slider('Gain (dBi)',-3,9,2,1,key='ant_gain');antenna_polarization=st.selectbox(_r,[_s,_t,'Circular'],key='pol');antenna_pattern=st.selectbox('Pattern',[_u,'Directional'],key='pattern')
...
with st.sidebar.expander('📊 Receiver Specs',expanded=_A):
	...
	with col2:desense_penalty=st.slider('Desense (dB)',0,25,6,1,key=_v,help='Desensitization from nearby transmitters. Use filters to reduce.');snr_required=st.slider('Req SNR (dB)',6,20,6,1,key='snr_req',help='Signal-to-Noise Ratio required for reliable decoding')
...
with st.sidebar.expander('🌍 Environment & Model',expanded=_A):
	propagation_model=st.selectbox(_Z,[_a,_K,_S,_G,_F,_E],index=4,format_func=lambda x:{_a:'Simple (Path Loss Exp.)',_K:'Free Space (Ideal)',_S:'Two-Ray Ground',_G:'Okumura-Hata (Urban/Suburban)',_F:'ITU-R P.1546 (VHF/UHF)',_E:'Blended Model (Industry Standard)'}[x],key='prop_model')
...
with tab1:
	...
	with col_map:
		...
		if st.session_state.drone_location is not _R:folium.Marker(st.session_state.drone_location,popup=f"""
                ...
                """,tooltip='Drone Repeater Station',icon=folium.Icon(color=_M,icon='broadcast-tower',prefix='fa')).add_to(m);folium.Circle(st.session_state.drone_location,radius=range_2m*1000,popup=f"2m Band: {range_2m:.1f} km",color=_J,fill=_A,fillColor=_J,fillOpacity=.15,weight=2,dashArray='5, 5').add_to(m);folium.Circle(st.session_state.drone_location,radius=range_70cm*1000,popup=f"70cm Band: {range_70cm:.1f} km",color=_N,fill=_A,fillColor=_N,fillOpacity=.1,weight=2,dashArray='10, 5').add_to(m);folium.Circle(st.session_state.drone_location,radius=system_range*1000,popup=f"System Range: {system_range:.1f} km",color=_O,fill=_A,fillColor=_O,fillOpacity=.2,weight=3).add_to(m);folium.Circle(st.session_state.drone_location,radius=radio_horizon*1000,popup=f"Radio Horizon: {radio_horizon:.1f} km",color=_U,fill=_H,weight=1,dashArray='2, 5').add_to(m)
...
with tab3:
	...
	st.markdown('**🎯 Propagation Model Comparison**');models=[_K,_S,_G,_F,_E];model_names=['Free Space','Two-Ray','Okumura-Hata','ITU-P.1546','Blended'];model_ranges=[]
	for model in models:
...
with tab4:
	st.markdown('**Optimization Recommendations**');recommendations=[];scores=[];vhf_uhf_ratio=range_2m/range_70cm if range_70cm>0 else 10;altitude_factor=min(drone_altitude/1e2,_B)
	...
	if tx_power_2m<_B:recommendations.append(f"• **Power**: {tx_power_2m}W is low. Increase to 1.5-2.0W for optimal efficiency");scores.append(1)
...
with tab6:
	...
	st.markdown(_T);col_guide,col_tech=st.columns(2)
	with col_guide:
...
- **Altitude Effect**: Exponential reduction: e^(-2.5*alt/100) = {np.exp(-2.5*min(drone_altitude/1e2,_B)):.2f} at {drone_altitude}m
- **Path Loss Difference**: VHF vs UHF: Free space = 9.7 dB, Actual = {calculate_path_loss_db(_B,freq_2m,2.,drone_altitude,_K)-calculate_path_loss_db(_B,freq_70cm,2.,drone_altitude,_K):.1f} dB at 1 km
