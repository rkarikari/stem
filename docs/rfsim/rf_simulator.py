...
with st.sidebar:
    ...
    st.markdown(f'<div style="font-size:0.72rem;line-height:1.9">'
                f'<span class="{p_b}">pyttsx3</span> {py3_msg}<br>'
                f'<span class="{g_b}">gTTS</span> {gt_msg}<br>'
                f'<span class="badge-ok">formant</span> Always available</div>',
                unsafe_allow_html=True)
    st.markdown("---")
...
if res is None:
    ...
    # Tabs
    t1,t2,t3,t4,t5=st.tabs(["OVERVIEW","SPECTRUM","WAVEFORMS","LEVEL INSPECTOR","DATA TABLE"])
    ...
    with t4:
        ...
        # OSC_SAMPLES: downsample to ~2000 points for oscilloscope (real audio samples)
        # BAR_BINS: 60 RMS windows for bargraph (real per-window RMS)
        OSC_PTS  = 2000
        BAR_BINS = 60
        _level_data = []
        for _n in res['levels']:
            ...
            _seg  = res['rf_segs'][_n]
            # --- Real oscilloscope samples ---
            _ds = max(1, len(_seg) // OSC_PTS)
            _osc_raw = _seg[::_ds][:OSC_PTS]
            _osc = [round(float(v), 5) for v in _osc_raw]   # list of float in [-1,1]
            # --- Real RMS bargraph windows ---
            _wsize = max(1, len(_seg) // BAR_BINS)
            _rms_bins = []
            for _i in range(BAR_BINS):
                _chunk = _seg[_i*_wsize : min((_i+1)*_wsize, len(_seg))]
                if len(_chunk) > 0:
                    _rms_bins.append(round(float(np.sqrt(np.mean(_chunk**2))), 5))
                else:
                    _rms_bins.append(0.0)
            # Normalise RMS bins to 0-1 against global max across ALL levels for fair comparison
            ...
            _level_data.append({
                ...
                'osc':   _osc,          # real downsampled audio waveform
                'rms':   _rms_bins,     # real per-window RMS values (raw, not normalised)
                'wav':   _wav_b64,
                'color': SC.get(_n, '#546e7a'),
            })

        # Normalise rms across all levels so bargraph is comparable
        _all_rms = [v for d in _level_data for v in d['rms']]
        _global_max = max(_all_rms) if _all_rms else 1.0
        for _d in _level_data:
            _d['rms_norm'] = [round(v/_global_max, 5) for v in _d['rms']]
...
// ── Oscilloscope — real audio samples, scrolling playhead ──
const oc=document.getElementById('osc-canvas');
const ox=oc.getContext('2d');
const OW=oc.width,OH=oc.height;
// oscPlayhead: index into current level's osc array, advances each tick
let oscPlayhead=0;
// oscWindow: the visible window of samples (OW wide)
const OSC_WIN=OW;

function drawOsc(col){
  const d=LEVELS.find(x=>x.level===currentLevel);
  if(!d||!d.osc||d.osc.length===0)return;
  const samples=d.osc;
  const N=samples.length;
  // Advance playhead (wraps around so oscilloscope scrolls continuously)
  oscPlayhead=(oscPlayhead+2)%N;
  ...
  // Plot OSC_WIN samples starting from playhead (wrap-around)
  // We map OSC_WIN samples across the full canvas width
  const visN=Math.min(OSC_WIN, N);
  const step=OW/(visN-1);
  ox.beginPath();
  for(let i=0;i<visN;i++){
    const si=(oscPlayhead+i)%N;
    const sample=samples[si];
    // Add mild noise if noise mode on
    const nz=noiseActive?(Math.random()-.5)*.04:0;
    const sv=Math.max(-1,Math.min(1,sample+nz));
    const x=i*step;
  ...
  ox.strokeStyle='#4fc3f730'; ox.lineWidth=1;
  ox.beginPath(); ox.moveTo(OW-2,0); ox.lineTo(OW-2,OH); ox.stroke();
}

// ── Bargraph — real RMS windows from audio ──
...
function drawBar(col){
  const d=LEVELS.find(x=>x.level===currentLevel);
  if(!d||!d.rms_norm||d.rms_norm.length===0)return;
  const bins=d.rms_norm;
  const N=bins.length;
  ...
  bins.forEach((v,i)=>{
    ...
    // Colour by normalised level
    const bc2=v<.33?'#26c6da':v<.66?'#7e57c2':'#ef5350';
    // Gradient bar
    const bg=bx.createLinearGradient(x,y,x,BH);
    bg.addColorStop(0,bc2+'ff');bg.addColorStop(1,bc2+'44');
    bx.fillStyle=bg;
    bx.fillRect(x,y,Math.max(bw,1),bh);
  });
  // Peak line
  const pH=peakBin*(BH*.92);
