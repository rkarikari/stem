...
class SchoolAI:
    ...
    def _load_openrouter_key(self) -> str:
        """Load OpenRouter API key from secrets.toml"""
        try:
            if hasattr(st, 'secrets') and 'openrouter' in st.secrets:
                return st.secrets.openrouter.get('api_key', '')
            else:
                with open('.streamlit/secrets.toml', 'r') as f:
                    secrets = toml.load(f)
                    return secrets.get('openrouter', {}).get('api_key', '')
        except Exception:
            return st.session_state.get('openrouter_key', '')
...
def main():  
    ...
    if 'ai_provider' not in st.session_state:
        ...
        try:
            test_response = requests.get('http://localhost:11434/api/tags', timeout=1)
            is_ollama_available = test_response.status_code == 200
        except:
            is_ollama_available = False
        
        # Default to cloud provider if Ollama not available
        st.session_state.ai_provider = AIProvider.OLLAMA if is_ollama_available else AIProvider.OPENROUTER
    ...
    # Provider selection with silent validation
    ...
    # Only show Local option if Ollama is available
    if is_ollama_available:
        provider_options = ["Local", "Cloud"]
        default_index = 0 if st.session_state.ai_provider == AIProvider.OLLAMA else 1
    else:
        provider_options = ["Cloud"]
        default_index = 0
        # Silently switch to cloud if local was selected but not available
...
def dashboard_page():
    ...
    with tab5:
        ...
        with health_col3:
            ...
            st.plotly_chart(fig_gpa_gauge, use_container_width=True)
        ...
        with health_col4:
            ...
            st.plotly_chart(fig_att_gauge, use_container_width=True)
...
#---------------------------------------------    
    ...
    with col2:
        if st.button("🎯 Identify At-Risk Students", use_container_width=True):
            if students_data:
                at_risk_students = []
                for student in students_data:
    ...
    with col2:
        ...
        if streams_data:
            ...
            st.plotly_chart(fig_util_mini, use_container_width=True)
        else:
...
def academics_page():
    ...
    with tab2:
        ...
        with st.expander("📊 GPA Scale Reference", expanded=False):
            ...
            st.dataframe(gpa_scale_df, use_container_width=True, hide_index=True)
        ...
        with grade_tab1:
            ...
            if recent_grades:
                ...
                st.dataframe(styled_grades, use_container_width=True)
