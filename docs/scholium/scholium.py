...
_CU='school_ai'
_CT='school_manager'
_CS="SELECT COUNT(*) FROM teachers WHERE status = 'active'"
_CR="SELECT COUNT(*) FROM students WHERE status = 'active'"
_CQ="UPDATE students SET status = 'graduated' WHERE student_id = ?"
_CP='UPDATE students SET grade = ? WHERE student_id = ?'
_CO='assignment_method'
_CN='activities'
_CM='previous_gpa'
_CL='required_fields'
_CK='max_age_deviation'
_CJ='utilization_percent'
_CI='UPDATE streams SET class_teacher_id = ? WHERE stream_id = ?'
_CH='Authorization'
_CG='Content-Type'
_CF='https://openrouter.ai/api/v1/models'
_CE='✅ Data restored successfully!'
_CD='.json'
_CC='Transaction Type'
_CB='selected_currency_name'
_CA='Credits'
_C9='Course ID'
_C8='Is Class Teacher'
_C7='Total Streams'
_C6='Hire Date'
_C5='Salary'
_C4='Admission Date'
...
_C2='Balanced'
_C1='Random'
_C0='Performance-based'
_B_='Medical Information'
_Bz='capacity'
_By='Current'
_Bx='Utilization'
_Bw='RdYlGn'
_Bv='Total Transactions'
_Bu='Tuition'
_Bt='At-Risk Students'
_Bs='Average Attendance'
_Br='None selected'
_Bq='context'
_Bp='infrastructure'
_Bo='report'
...
class SchoolAI:
	...
	def get_openrouter_free_models(D):
		...
		try:
			A=requests.get(_CF,headers={_CG:_AQ},timeout=10)
	...
	def _test_connection(A):
		...
		try:
			if A.provider_str==_g:B=requests.get(f"{A.ollama_url}/api/tags",timeout=5);return C if B.status_code==200 else D
				...
				B=requests.get(_CF,headers={_CH:f"Bearer {A.openrouter_key}"},timeout=5);return C if B.status_code==200 else D
	...
	def _openrouter_request(B,prompt,context):
		...
		try:
			...
			G=f"{D}\n\n{C}"if D else C;A=requests.post('https://openrouter.ai/api/v1/chat/completions',headers={_CH:f"Bearer {B.openrouter_key}",_CG:_AQ},json={_Bh:B.model,'messages':[{_At:'user',_Aa:G}]},timeout=60)
...
class StreamManager:
	...
	def assign_class_teacher(B,stream_id,teacher_id):'Assign class teacher to a stream';C=teacher_id;A=_CI;B.db.execute_update(A,(C,stream_id));A='UPDATE teachers SET is_class_teacher = TRUE WHERE teacher_id = ?';B.db.execute_update(A,(C,))
	...
	def get_stream_statistics(D):
		...
		for A in E:
			...
			B[_AR].append({_O:A.stream_id,_H:A.grade_level,_w:A.stream_type,_Ax:F,_y:A.max_capacity,_CJ:round(I,1)})
		return B
...
class SchoolManager:
	def __init__(A):A.db=DatabaseManager();A.ai=SchoolAI();A.stream_manager=StreamManager(A.db);A.admission_criteria={_AA:2.,_CK:2,_CL:[_E,_Z,_AB]}
	def auto_admit_student(A,application_data):
		...
		try:
			I=[]
			for N in A.admission_criteria[_CL]:
			...
			E=B.get(_Z,6);D=A._determine_grade_by_age(E);O=A._get_typical_age_for_grade(D);R=abs(E-O);C=B.get(_CM,3.)
			if isinstance(C,str):
				try:C=float(C)
			...
			if R>A.admission_criteria[_CK]:return{_I:_An,L:f"Age {E} not appropriate for available grade levels",_Am:f"Student age ({E}) significantly outside typical range for grade placement"}
			S=f"""
            ...
            - Extracurricular Activities: {B.get(_CN,P)}
            ...
            ✓ GPA ({C}) meets minimum requirement ({A.admission_criteria[_AA]})
	...
	def _update_student_grade(A,student_id,new_grade):"Update student's grade in database";B=_CP;A.db.execute_update(B,(new_grade,student_id))
	def _graduate_student(A,student_id):'Mark student as graduated';B=_CQ;A.db.execute_update(B,(student_id,))
...
class AIHelperFunctions:
	...
	def compile_context_data(A):
		...
		try:B=A.db.execute_query(_CR);C=A.db.execute_query(_CS);D=A.db.execute_query(_BF);E=B[0][0]if B and len(B)>0 else 0;F=C[0][0]if C and len(C)>0 else 0;G=D[0][0]if D and len(D)>0 else 0;H=f"""
            SCHOOL OVERVIEW:
            - Active Students: {E}
            - Active Teachers: {F}
            - Total Courses: {G}
...
def main():
	...
	if _CT not in st.session_state:st.session_state.school_manager=SchoolManager()
	...
	if _CU not in st.session_state:st.session_state.school_ai=SchoolAI()
	...
	if st.session_state.ai_provider==AIProvider.OLLAMA:
		if st.session_state.get(F)!=D:
			...
			with st.sidebar.expander('Server Configuration',expanded=_F):G=st.text_input('Ollama Server URL',value=st.session_state.get(_q,_B9),placeholder=_B9,help='Change if running Ollama on different host/port')
		...
		with H:
			...
			if C:
				N=[f"{A.get(_E,A['id'])}"for A in C];E=[A['id']for A in C];A=st.session_state.get(_Y)
	...
	st.title('🏫 RadioSport Scholium');Y,Z,a,b,c,d,e,f,g,h=st.tabs(['📊 Dashboard','👥 Admissions','🎓 Students',_CV,'📚 Academics','💰 Finance','📈 Analytics','🤖 AI Insights','🏫 Streams',_CW])
...
def dashboard_page():
	...
	with S:st.metric(_Bt,A3,delta=f"{A3/G*100:.1f}%"if G>0 else AA)
	...
	with Bv:
		E,F=st.columns(2)
		with E:
			if A4>0 or A5>0:C9=pd.DataFrame({BD:[_Bu,'Fees'],_AT:[A4,A5]});Ai=px.pie(C9,values=_AT,names=BD,title='Revenue Breakdown',color_discrete_sequence=[BE,'#2196F3']);Ai.update_layout(height=400);st.plotly_chart(Ai,use_container_width=_A)
		with F:
	...
	with E:
		if st.button('📊 Generate Analytics Report',type=_AW,use_container_width=_A):
			with st.spinner('Generating comprehensive analytics report...'):
				I={g:{_BP:G,_Ab:round(l,2),BN:A2,BO:round(A2/G*100,1)if G>0 else 0,BP:A3,BQ:round(A3/G*100,1)if G>0 else 0},AH:{BR:round(m,1),BS:len([A for A in B.values()if A>=95])if B else 0,BT:len([A for A in B.values()if A<75])if B else 0},AS:{'total_teachers':a,BU:round(L,1),_AR:round(T,1)if D else 0},A1:{BV:b,BW:A4,BX:A5,'total_transactions':R,BY:round(b/R,2)if R>0 else 0},BM:{'total_courses':AI,'active_streams':AX,BZ:round(Aw,1)if k and k[0][0]else 0}};st.success('✅ Analytics report generated successfully!')
				with st.expander('📋 Comprehensive Analytics Report',expanded=_A):
					...
					with r:st.metric(_Bt,f"{I[g][BP]} ({I[g][BQ]}%)")
					st.markdown('### 📅 Attendance Analysis');U,V,r=st.columns(3)
					with U:st.metric(_Bs,f"{I[AH][BR]}%")
					...
					st.markdown('### 📥 Export Options');CM=json.dumps(I,indent=2);st.download_button(label='📥 Download Report as JSON',data=CM,file_name=f"school_analytics_report_{pd.Timestamp.now().strftime(_A7)}.json",mime=_AQ)
	with F:
		if st.button('🎯 Identify At-Risk Students',use_container_width=_A):
			if A:
				d=[]
				for K in A:
	...
	with S:
		if st.button('💰 Financial Summary',use_container_width=_A):
			...
			with st.expander(_Ci):
				U,V=st.columns(2)
				with U:st.metric(Ba,f"{N}{A4:,.2f}");st.metric(_Bv,R)
	...
	with n:
		if st.button('🔄 System Health Check',use_container_width=_A):
			with st.spinner('Running system health check...'):
				...
				if Z:
					...
					with st.expander('View Health Issues'):
						...
						CR=['Data Integrity','Academic Performance',_AF,'System Resources'];CS=[100 if A and O and D else 50,100-AM/G*100 if A else 100,m if B else 100,min(100,100-T)if D else 100];CT=pd.DataFrame({AB:CR,AT:CS});A_=px.bar(CT,x=AB,y=AT,title='System Health Breakdown',color=AT,color_continuous_scale=_Bw);A_.add_hline(y=75,line_dash=_AD,line_color=BJ,annotation_text='Warning Threshold');st.plotly_chart(A_,use_container_width=_A)
				else:
					st.success('✅ All systems operating normally!');CU={'System Uptime':'99.9%','Data Quality':_A_,_N:BL,'Security':'Secure'}
	...
	with F:
		...
		if D:CX=pd.DataFrame({_Aq:[f"Stream {A[0]}"for A in D[:5]],_Bx:[A[4]/A[3]*100 if A[3]>0 else 0 for A in D[:5]]});AO=px.line(CX,x=_Aq,y=_Bx,title='Stream Utilization Trend',markers=_A);AO.add_hline(y=100,line_dash=_AD,line_color=_t);AO.update_layout(height=300);st.plotly_chart(AO,use_container_width=_A)
	...
	with CY:
		...
		if D:Y=sum(A[3]for A in D);i=sum(A[4]for A in D);Cb=i*1.05;Cc=pd.DataFrame({Bf:[_By,'Projected (Next Term)'],AU:[i,min(Cb,Y)]});B2=px.bar(Cc,x=Bf,y=AU,title='Enrollment Projection',color=AU);B2.add_hline(y=Y,line_dash=_AD,line_color=_t,annotation_text=_BO);st.plotly_chart(B2,use_container_width=_A)
	with CZ:
	...
	if A or O:
		...
		if P:
			...
			with Cg:
				Ci=['Academic',_AF,'Capacity','Financial'];B9=[len([A for A in P if'🚨'in A or _h in A]),len([A for A in P if _AE in A.lower()]),len([A for A in P if _Bz in A.lower()or'streams'in A.lower()]),len([A for A in P if'💰'in A or'revenue'in A.lower()])]
...
def admissions_page():
	...
	with Z:
		...
		if _CT not in st.session_state:st.error('School management system not initialized. Please restart the application.');return
		...
		with st.form('admission_form'):
			...
			e=st.number_input('Previous School GPA',min_value=_D,max_value=4.,value=3.);f=st.text_area('Extracurricular Activities',placeholder='List activities');T=st.text_input('Parent Contact*',placeholder=_Ck);g=st.text_area(_B_,placeholder='Any medical conditions');h=st.form_submit_button('🤖 AI Auto-Evaluate Application');i=st.selectbox('Stream Assignment Method',[_C0,_C1,_C2],help='How students are assigned to class streams',index=2)
			if h:
				D=[]
				...
				if D:
					...
					from datetime import datetime;k={_E:S,_Z:C,_Aj:B.strftime(_M),_CM:e,_CN:f,_AB:T,_A9:g,_r:datetime.now().strftime(_M),_CO:i.lower().replace('-','')}
					with st.spinner('AI is evaluating the application...'):
						try:
							...
							if G==_BC:
								st.success('🎉 Application ACCEPTED!');st.write(f"**Student ID:** {J.get(_C,_i)}");st.write(f"**Assigned Grade:** {J.get(_H,_i)}");st.write(f"**Date of Birth:** {B.strftime('%B %d, %Y')}");st.write(f"**Age:** {C} years")
	...
	with a:
		...
		st.subheader('📈 Quick Stats');o,p=st.columns(2);K=sum(I[_e])
		...
		if hasattr(A,'ai')and A.ai:
			...
			st.write(f"**Model:** {st.session_state.get(_Y,_Br)}");st.write(f"**Provider:** {L}");st.write(f"**Connection Status:** {A.ai._test_connection()}")
...
def students_page():
	...
	w,x=st.tabs(['📊 View Students','✏️ Edit Student Data']);C=pd.DataFrame(S);C[_As]=C.apply(lambda row:f"{row.get(_O,_i)} ({row.get(_A5,_i)})"if row.get(_O)and row.get(_A5)else _A2,axis=1)
	with w:
		...
		if not B.empty:
			...
			with H:st.subheader(f"📋 {A[_E]} Details");st.write(f"**Student ID:** {A[_C]}");st.write(f"**Grade:** {A[_H]}");st.write(f"**Age:** {A[_Z]}");st.write(f"**GPA:** {A[_G]}");st.write(f"**Attendance:** {A[_K]}%");st.write(f"**Behavior Score:** {A[_U]}");st.write(f"**Stream:** {A[_As]}");st.write(f"**Status:** {A.get(_I,'Active')}")
			with I:st.subheader('📊 Performance Chart');W={_BR:[_h,_AF,'Behavior'],q:[A[_G]*25,A[_K],A[_U]],r:[100,100,100]};X=go.Figure();X.add_trace(go.Bar(name=_By,x=W[_BR],y=W[q]));X.add_trace(go.Bar(name='Maximum',x=W[_BR],y=W[r],opacity=.3));X.update_layout(title='Student Performance Metrics',barmode='overlay');st.plotly_chart(X,use_container_width=_A)
	with x:
		...
		if m:
			...
			with st.form('edit_student_form'):
				...
				with H:
					st.markdown('**📚 Academic Information**');A1=st.number_input(_h,min_value=_D,max_value=4.,value=float(A[_G]),step=.1,format='%.2f');A2=st.selectbox(_A1,options=sorted(C[_H].unique()),index=list(sorted(C[_H].unique())).index(A[_H]));D=A.to_dict();A3=C[_As].unique();e=[A for A in A3 if A!=_A2];F=D.get(_O);Q=D.get(_A5)
					if F and Q:G=f"{F} ({Q})"
					elif F:G=F
					else:G=_A2
				...
				with I:st.markdown('**📊 Performance Metrics**');A6=st.number_input(_B1,min_value=_D,max_value=1e2,value=float(A[_K]),step=.1,format=s);A7=st.number_input(_BS,min_value=_D,max_value=1e2,value=float(A[_U]),step=.1,format=s);A8=st.selectbox('Student Status',options=[_n,_BT,t,_AK],index=[_n,_BT,t,_AK].index(A.get(_I,_n)))
				with T:
					st.markdown('**👨\u200d👩\u200d👧\u200d👦 Personal Information**');a=A.get(_Aj);n=A.get(_Z,10)
					if a:
						try:
					...
					g=st.date_input('Date of Birth',value=b,min_value=datetime.now().date()-timedelta(days=9125),max_value=datetime.now().date()-timedelta(days=1095),help='Age will be automatically calculated from this date');J=calculate_age(g);A9=int(A[_Z])
					if J!=A9:st.write(f"**Updated age:** {J} years")
					else:st.write(f"**Age:** {J} years")
					AA=st.text_input(_C3,value=A.get(_AB,''),placeholder='Phone number or email');AB=st.text_area(_B_,value=A.get(_A9,''),height=80,placeholder='Any relevant medical information...')
			...
			with AJ:o=D.get(_Z,_Ai);st.metric(_AS,f"{o} years"if o!=_Ai else _Ai);p=D.get(_r,u);st.metric(_C4,p if p else u)
def teachers_page():
	...
	try:
		...
		if not O:
			...
			with st.form('add_teacher'):
				...
				with E:B8=st.selectbox(_Q,[s,AM,_BU,t,Ax,u]);B9=st.slider(_AV,_D,1e2,85.);BA=st.number_input(_C5,min_value=_D,value=5e4)
				BB=st.date_input(_C6);w=st.checkbox('Is Class Teacher?',value=_F)
			...
			return
		x={_AI:[A[0]for A in O],_P:[A[1]for A in O],_m:[A[2]for A in O],_Q:[A[3]for A in O],_N:[A[4]for A in O],_o:[A[5]for A in O],_A3:[Q if A[6]else'❌'for A in O],K:[A[7]for A in O]};AO=[]
		for I in x[_AI]:
	...
	with BF:
		...
		with D:
			...
			elif AR==A_:G=G[G[_A3]=='❌']
			BL=[_AI,_P,_m,_Q,_N,_o,_A3,K,_J];G=G[BL];st.dataframe(G,use_container_width=_A)
			if st.button('📥 Export Teacher Data'):T=G.to_csv(index=_F);st.download_button(label='Download CSV',data=T,file_name='teachers_data.csv',mime=_Ar)
		with E:
		...
		with BN:
						...
						try:
							...
							if AU:
                                ...
                                **Selected Stream Details:**
                                ...
                                - **{BU}**
                                """)
								if J[_Q]in[s,u]or _Af in J[_m]or'Math'in J[_m]:st.success("✅ Teacher's background is suitable for this stream assignment")
								...
								if st.button('Assign Stream',key='assign_btn'):
									...
									try:A3=_CI;C.execute_update(A3,(I,AW));st.success(f"✅ Successfully assigned {A0} ({J[_Q]}) to {AY}");st.rerun()
			...
			with E:
				...
				if len(A)>0:
					...
					if V>0:
						...
						for(l,B)in BW.iterrows():st.write(f"• **{B[_P]}** - {B[_Q]} ({B[_m]})")
					st.write('**Class Teachers by Department:**');BX=N[_Q].value_counts()
					for(d,BY)in BX.items():BZ=len(N[(N[_Q]==d)&(N[K]!=a)]);st.write(f"• **{d}:** {BY} total ({BZ} assigned)")
			...
			try:
				...
				if Y:
					...
					with D:Ad=len(Y);st.metric(_C7,Ad)
				...
				try:
					...
					if n:
						...
						for Bn in n:
							...
							if not A5.empty:H=A5.loc[A5[_N].idxmax()];st.write(f"**{U} - {M}**");st.write(f"→ Suggested: {H[_P]} ({H[_Q]}, {H[_N]:.1f}%)")
				...
				with BO:
					...
					with D:
						...
						try:
							...
							st.info(f"📚 Found {len(Z)} subjects: {_j.join(sorted(Z)[:5])}{'...'if len(Z)>5 else''}");X=st.selectbox(_m,sorted(Z),key='ai_subject');U=st.selectbox(_A1,[_a,'Lower Primary','Upper Primary','Junior High School'],key='ai_grade');A6=st.number_input('Expected Class Size',min_value=1,max_value=50,value=35,key='ai_class_size')
							if st.button('🎯 AI Recommend Teacher',key='ai_recommend'):
								try:
									...
									if Aj:
										P=pd.DataFrame(Aj,columns=[_AI,_P,_m,_Q,_N,_C8,_J,K]);P[B5]=1-P[_J]/200;P[g]=P[_N]*.7+P[B5]*100*.3;H=P.loc[P[g].idxmax()];p=int(H[_J])
										if p+A6<=200:st.success(f"**Recommended Teacher: {H[_P]}**");st.write(f"- Teacher ID: {H[_AI]}");st.write(f"- Subject: {H[_m]}");st.write(f"- Performance Score: {H[_N]:.1f}%");st.write(f"- Current Students: {p}");st.write(f"- Department: {H[_Q]}");st.write(f"- Class Teacher: {'Yes'if H[_C8]else'No'}");st.write(f"- Assigned Stream: {H[K]}");st.write(f"- Recommendation Score: {H[g]:.1f}");st.info('**Recommendation Reasoning:**');st.write(f"- High performance score ({H[_N]:.1f}%)");st.write(f"- Manageable current workload ({p} students)");st.write(f"- Can accommodate {A6} additional students")
										else:
											...
											if not Ak.empty:
												...
												for(C3,B)in Ak.head(3).iterrows():st.write(f"• **{B[_P]}** (Score: {B[g]:.1f}, Students: {int(B[_J])}, Stream: {B[K]})")
					...
					with E:
						...
						if len(A)>0:
							Am=px.bar(A,x=_P,y=_J,title='Teacher Workload (Students per Teacher)',color=_N,color_continuous_scale=_Bw);Am.update_xaxes(tickangle=45);st.plotly_chart(Am,use_container_width=_A);st.write('**Workload Statistics:**');st.write(f"- Average students per teacher: {A[_J].mean():.1f}");st.write(f"- Max students (single teacher): {A[_J].max()}");st.write(f"- Min students (single teacher): {A[_J].min()}");An=A[A[_J]>150]
							if len(An)>0:
								st.warning('⚠️ **Overloaded Teachers:**')
								for(l,B)in An.iterrows():st.write(f"• {B[_P]}: {B[_J]} students")
...
def academics_page():
	...
	with BW:
		...
		with st.expander(_Cp):
			with st.form('add_course_form'):
				G,H=st.columns(2)
				with G:f=st.text_input(_C9);V=st.text_input(_BW);Av=st.selectbox(_A1,[_R,_S,_T,_V,_W,_X,_b,_c,_d])
				with H:
					...
					credits=st.number_input(_CA,min_value=1,max_value=6,value=3);Ba=st.text_input('Schedule (e.g., MWF 9:00-10:00)')
		...
		if X:Bd=pd.DataFrame(X,columns=[_C9,_BW,_B6,_A1,_CA,_B7,'Enrolled Students']);st.dataframe(Bd,use_container_width=_A,hide_index=_A)
	...
	with BX:
		...
		with st.expander('📊 GPA Scale Reference',expanded=_F):Bg=[[AI,'75-100','4.0',_A_],[AJ,'70-74','3.5',Al],[AK,'65-69','3.0',_Ap],[AL,'60-64','2.5',O],[AM,'55-59','2.0',O],[AN,'50-54','1.5',O],[AO,'45-49','1.0',e],[AP,'40-44','0.5',e],[AQ,'0-39','0.0',AR]];Bh=pd.DataFrame(Bg,columns=[_f,'Percentage Range',BN,BO]);st.dataframe(Bh,use_container_width=_A,hide_index=_A)
		with Be:
			G,H=st.columns(2)
			with G:
		...
		with Bf:
			...
			with G:
				...
				if st.button('🔄 Update All Student GPAs',type=_AW):
			...
			st.markdown(_AP);st.markdown('### 👤 Individual Student GPA Analysis');Y=_BE;M=A.execute_query(Y)
			if M:
				...
				if Ai!=AU:
					...
					st.markdown(_AP);st.markdown('#### 📈 GPA Trend Analysis')
					try:
						L=U.get_gpa_by_semester(D)
						if L and len(L)>1:
							B3=[]
			...
			st.markdown(_AP);st.markdown('### 🏆 GPA Leaderboard')
			try:
	...
	with BZ:
		...
		st.markdown(_AP);st.markdown('### 👥 Individual Student Review');G,H=st.columns([2,1])
		with G:
...
"""
		...
		with H:
			...
			if st.button('🚀 Run Bulk AI Promotion',type=_AW):
				with st.spinner('Processing AI-powered promotions for all students...'):
					try:
						...
						for E in BA:
							...
							if BE and BF and BG:
								A0=get_next_grade(N)
								if A0==_AJ:A.execute_update(_CQ,(D,));BD+=1;k=_AK;l=_AJ;m='Met all promotion criteria - Graduated'
								else:A.execute_update(_CP,(A0,D));BB+=1;k=_Bm;l=A0;m='Met all promotion criteria'
			...
			st.markdown(_AP);st.markdown('### 📊 Historical Promotion Analysis')
			try:
...
def finance_page():
	...
	if _CB in st.session_state:J=st.session_state.selected_currency_name;A=A2.get(J,'₵')
	else:J=_Ba;A='₵'
	...
	with A4:
		...
		with D:st.subheader('📝 Transaction Details');M=st.selectbox(_CC,[a,b,y,z,_x,'utilities','supplies','maintenance','other_expenses']);N=st.number_input(f"Amount ({A})",min_value=_D,step=.01,format='%.2f');R=st.text_input(_Bb,placeholder='Enter transaction description...')
		with E:
		...
		with D:
			if st.button('💾 Save Transaction',type=_AW,use_container_width=_A):
		...
		with E:
			if st.button('🔄 Clear Form',use_container_width=_A):st.rerun()
		with AP:
			if st.button('📊 View Overview',use_container_width=_A):st.info("💡 Switch to the 'Financial Overview' tab to see updated metrics and charts!")
		...
		with AW:
			...
			try:
				...
				if V:
					...
					if r:
						st.subheader('⚠️ Overdue Payments');O=pd.DataFrame(r,columns=[_AG,_AT,_Bb,c]);O[_AT]=O[_AT].apply(lambda x:f"{A}{x:,.2f}"if pd.notnull(x)else f"{A}0.00")
...
def analytics_page():
	...
	with p:
		...
		with D:x=A[_K].mean();st.metric(_Bs,f"{x:.1f}%")
		with G:y=len(A[A[_G]>=3.5]);st.metric('High Performers (GPA ≥ 3.5)',y)
		with S:z=len(A[(A[_G]<2.)|(A[_K]<75)]);st.metric(_Bt,z)
		st.divider();st.subheader('📊 System Overview');C,D,G,S=st.columns(4)
		try:
			with C:A0=F.execute_query(_CR)[0][0];st.metric('Active Students',A0)
			with D:A1=F.execute_query(_CS)[0][0];st.metric(_CZ,A1)
			with G:A2=F.execute_query(_BF)[0][0];st.metric(_Ca,A2)
			with S:A3=F.execute_query('SELECT COUNT(*) FROM streams')[0][0];st.metric(_C7,A3)
	...
	with r:
		...
		if U:
			...
			with D:st.subheader('Stream Details');J=I[[_O,_A8,_A5,_Ax,_y,j]].copy();J.columns=[_Ad,_f,_AX,_By,_BO,_B6];st.dataframe(J,use_container_width=_A,hide_index=_A)
	...
	with s:
		...
		if V:
			...
			with D:B=px.bar(N,x=_w,y=a,title='Transaction Count by Type',labels={_k:_CC,_l:_e});st.plotly_chart(B,use_container_width=_A)
			st.divider();C,D,G=st.columns(3);g=N[Z].sum();W=N[a].sum();A7=g/W if W>0 else 0
			with C:st.metric(_BM,f"${g:,.2f}")
			with D:st.metric(_Bv,W)
			with G:st.metric('Avg Transaction Value',f"${A7:.2f}")
			st.subheader(_Ci);J=N.copy();J.columns=[_CC,b,'Transaction Count'];J[b]=J[b].apply(lambda x:f"${x:,.2f}");st.dataframe(J,use_container_width=_A,hide_index=_A)
	...
	with t:
		...
		if X:
			...
			with D:AB=len(K[K[_p]>=90]);st.metric('Top Performers (≥90)',AB)
			with G:AC=K[_x].mean();st.metric('Average Salary',f"${AC:,.2f}")
			st.subheader('📚 Department Analysis');R=K.groupby(_A4).agg({_p:_B5,_x:_B5,_E:_AL}).round(2);R.columns=[_BV,c,'Teacher Count'];R[c]=R[c].apply(lambda x:f"${x:,.2f}");st.dataframe(R,use_container_width=_A)
		...
		with C:
			...
			if st.button('🤖 Generate Performance Predictions',key='predict_performance'):
				try:
                    ...
                    - High Performers (GPA ≥ 3.5): {len(A[A[_G]>=3.5])}
...
def ai_insights_page():
	...
	with c:
		if st.button('🚀 Generate Complete AI Analysis',type=_AW):
	...
	with A5:
		if st.button('🔄 Clear AI Cache'):
			if _CU in st.session_state:del st.session_state.school_ai
	...
	with Bk:
		...
		with st.expander('➕ Add New Student',expanded=_F):
			with st.form('new_student_form'):
				L=st.text_input(_Cj,placeholder='Enter student name');Bt=st.number_input('Age*',min_value=3,max_value=25,value=6);Bu=st.selectbox(BR,[_a,_R,_S,_T,_V,_W,_X,_b,_c,_d]);Bv=st.number_input(_h,min_value=_D,max_value=4.,value=_D);Bw=st.number_input(_B1,min_value=_D,max_value=1e2,value=1e2);Bx=st.number_input(_BS,min_value=_D,max_value=1e2,value=1e2);By=st.text_input(_C3,placeholder=_Ck);Bz=st.text_area(_B_,placeholder='Any medical conditions or notes');t=st.date_input(_C4,value=datetime.now());U=st.form_submit_button('➕ Add Student')
				if U and L:
		...
		if k:
			...
			with T:
				...
				with st.form('delete_course_form'):
					AT=st.selectbox('Choose course to delete:',options=A1,key='course_delete_select');BF=st.checkbox('⚠️ I confirm I want to delete this course');Ag=st.form_submit_button('🗑️ Delete Course',type=_Bc)
		...
		with st.expander('➕ Add Attendance Record',expanded=_F):
			...
			with st.form('new_attendance_form'):
				...
				if U and L:
					...
					try:B.db.execute_update(D,E);st.success(f"✅ Attendance recorded for {L} on {l.strftime(_M)}!");st.rerun()
	...
	with Bp:
		...
		if AW:
			...
			with T:
				AC=A4[A4[AJ]]
				if not AC.empty:
					if st.button(f"🗑️ Delete Selected ({len(AC)} items)",key='delete_transactions',type=_Bc):st.session_state.transactions_to_delete=AC[R].tolist();st.session_state.confirm_delete=_A
				else:st.button('🗑️ Delete Selected (0 items)',key='delete_transactions_disabled',disabled=_A)
			if st.session_state.get(Aa,_F):
				Al=st.session_state.get(Ab,[]);AC=A4[A4[R].isin(Al)];st.warning(f"⚠️ Are you sure you want to delete {len(Al)} transaction(s)? This action cannot be undone.");st.write('**Transactions to be deleted:**');Am=AC[[R,W,S,_w,i,_L]].copy();Am[S]=Am[S].apply(lambda x:f"{a}{x:,.2f}");st.dataframe(Am,use_container_width=_A);CQ,CR=st.columns(2)
				with CQ:
					if st.button('✅ Confirm Delete',key='confirm_delete_btn',type=_AW):
						try:
							BO=0
		...
		with st.expander(_Cz,expanded=_F):
			...
			with st.form('new_transaction_form'):
				O,T=st.columns(2)
				with O:L=st.selectbox(_AG,list(J.keys()),help='Select a student (optional)');AE=st.number_input(f"Amount* ({a})",min_value=_D,value=_D,format='%.2f');CS=st.selectbox('Type*',[_Bu,'Fee',Ba,Bb,'Other'])
		...
		with CW:
			...
			if y is not _B:
				try:
					...
					elif y.name.endswith(_CD):F=pd.read_json(y)
					A5=[]
					...
					if BP==BA and not st.session_state.replace_confirmed:
						...
						if st.button('⚠️ Confirm Replace',key='confirm_replace'):st.session_state.replace_confirmed=_A;st.rerun()
				...
				with st.spinner('Creating backup...'):
					try:
						...
						if Ax==Bf:
							...
							Cg=json.dumps(z,indent=2,default=str);st.download_button(label='📥 Download JSON Backup',data=Cg,file_name=f"school_backup_{AY}.json",mime=_AQ)
						elif Ax==Bh:
							...
							AF=o.getvalue();st.download_button(label='📥 Download Excel Backup',data=AF,file_name=f"school_backup_{AY}.xlsx",mime=_Be)
...
def streams_page():
	...
	st.divider();st.subheader('📋 Stream Management');A=st.selectbox('Select Grade Level',[_a,_R,_S,_T,_V,_W,_X,_b,_c,_d],key='stream_grade_select');I=C.stream_manager.get_streams_for_grade(A);Ag=C.stream_manager.get_streams_config_for_grade(A)
	if not I:
		...
		return
	st.info(f"**{A}** has **{len(I)} streams**: {_j.join([A.stream_type for A in I])} | Class size limit: {C.stream_manager.max_class_size} students");Ah,Ai,Aj,Ak=st.tabs(['📊 Stream Overview','👥 Student Assignment','👨\u200d🏫 Teacher Assignment','⚙️ Capacity Management'])
	with Ah:
	...
	with Ai:
			...
			with F:
				if X:
					...
					if not st.session_state[f"confirm_reset_{A}"]:
						if st.button('🔄 Reset Student Assignments',key='reset_assignments',type=_Bc,help='This will unassign ALL students in this grade'):st.session_state[f"confirm_reset_{A}"]=_A;st.rerun()
					if st.session_state[f"confirm_reset_{A}"]:
						...
						with Ap:
							if st.button('✅ Confirm Reset',key='confirm_reset',type=_AW):
		...
		st.divider();Y=st.radio('Assignment Method',[_C0,_C1,_C2],index=2,key='stream_assignment_method',help='Performance-based: Assigns students based on GPA to appropriate streams. Random: Randomly distributes students. Balanced: Fills streams evenly.');AA={_C0:_Ak,_C1:_Bl,_C2:_Al};E,F=st.columns(2)
		with E:
	...
	with Ak:
				...
				try:
					...
					for G in B5:
						...
						for D in a:
							...
							if J>D.max_capacity:s.append({_O:D.stream_id,_H:G,Aa:J,_Bz:D.max_capacity,Ab:J-D.max_capacity})
					if s:
						st.warning(f"Found {len(s)} overcapacity streams:")
						for t in s:st.write(f"- **{t[_O]}**: {t[Aa]}/{t[_Bz]} ({t[Ab]} excess)")
		...
		try:
			...
			if I:
				...
				for D in I:J=C.stream_manager.get_student_count(D.stream_id);N=J/D.max_capacity*100;AM.append({_Aq:D.stream_id,_AX:D.stream_type,_J:J,'Capacity':D.max_capacity,_Bx:f"{N:.1f}%"})
		...
		with st.expander('🌍 School-wide Stream Statistics'):
			try:
				...
				if u[_AR]:
					...
					for A5 in u[_AR]:AN.append({_Aq:A5[_O],_f:A5[_H],_B0:A5[_CJ]})
...
def backup_restore():
	...
	with F:
		...
		if st.button('🔄 Create Backup'):
			...
			try:A={_AZ:C.get_all_students(),'backup_date':datetime.now().isoformat(),'version':'1.0'};I=json.dumps(A,indent=2);st.download_button(label='💾 Download Backup File',data=I,file_name=H.replace('.db',_CD),mime=_AQ);st.success('✅ Backup created successfully!')
			...
			if st.button('📥 Restore from Backup'):
				try:
					...
					if _AZ in A:
						...
						st.success(_CE);st.experimental_rerun()
