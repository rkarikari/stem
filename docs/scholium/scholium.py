...
_CD='✅ Data restored successfully!'
_CC='.json'
...
def main():
	...
	st.title('🏫 RadioSport Scholium');V,W,X,Y,Z,a,b,c,d,e=st.tabs(['📊 Dashboard','👥 Admissions','🎓 Students',_CV,'📚 Academics','💰 Finance','📈 Analytics','🤖 AI Insights','🏫 Streams',_CW])
...
def dashboard_page():
	...
	if A or O:
		...
		if P:
			...
			with Cg:
				Ci=['Academic',_AF,'Capacity','Financial'];B9=[len([A for A in P if'🚨'in A or _h in A]),len([A for A in P if _AE in A.lower()]),len([A for A in P if _By in A.lower()or'streams'in A.lower()]),len([A for A in P if'💰'in A or'revenue'in A.lower()])]
...
def students_page():
	...
	with w:
		...
		if not B.empty:
			...
			with I:st.subheader('📊 Performance Chart');W={_BR:[_h,_AF,'Behavior'],q:[A[_G]*25,A[_K],A[_U]],r:[100,100,100]};X=go.Figure();X.add_trace(go.Bar(name=_Bx,x=W[_BR],y=W[q]));X.add_trace(go.Bar(name='Maximum',x=W[_BR],y=W[r],opacity=.3));X.update_layout(title='Student Performance Metrics',barmode='overlay');st.plotly_chart(X,use_container_width=_A)
	with x:
							...
							if AU:
                                ...
                                **Selected Stream Details:**
								...
								if st.button('Assign Stream',key='assign_btn'):
									...
									try:A3=_CH;C.execute_update(A3,(I,AW));st.success(f"✅ Successfully assigned {A0} ({J[_Q]}) to {AY}");st.rerun()
...
def backup_restore():
	...
	with F:
		...
		if st.button('🔄 Create Backup'):
			...
			try:A={_AZ:C.get_all_students(),'backup_date':datetime.now().isoformat(),'version':'1.0'};I=json.dumps(A,indent=2);st.download_button(label='💾 Download Backup File',data=I,file_name=H.replace('.db',_CC),mime=_AQ);st.success('✅ Backup created successfully!')
